--[[
    ...
    ✅ Новая защита от повторного запуска (полный unload предыдущего экземпляра)
]]

-- ============================================================
-- UNLOAD PREVIOUS INSTANCE (всегда при повторном запуске)
-- ============================================================
if type(getgenv().Flumium_Unload) == "function" then
    pcall(getgenv().Flumium_Unload)
    print("[Flumium] предыдущий экземпляр полностью выгружен")
end
...
pcall(function()
    ...
    for _, e in ipairs(Lighting:GetChildren()) do
        if e.Name == "NVEffect" then e:Destroy() end
...
-- ============================================================
-- UNIFIED HOOK (Kunai Marker + Silent Aim)
-- ============================================================
...
task.spawn(function()
    while getgenv().Flumium_LoopsRunning do
        task.wait(1.5)
        local p = getPlayerPing()
        local dot = p < 100 and "🟢" or (p < 200 and "🟡" or "🔴")
        local cc = getClientCountry()
...
__regFn(function()
    if type(removeUnifiedHook) == "function" then pcall(removeUnifiedHook) end
end)
...
print("🛡️ Защита от повторного запуска: активна")
