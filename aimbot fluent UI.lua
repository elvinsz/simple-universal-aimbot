--[[
    Universal Shindo Cheat v8.5
    Исправлено: убран goto (не работает в Luau), MaxDistance, 360°, CHI/STAM, Force Reconnect
]]
...
local function searchStatInRoot(root, exactNames, excludeKeywords)
    ...
    for _, obj in ipairs(root:GetDescendants()) do
        if obj:IsA("NumberValue") or obj:IsA("IntValue") then
            ...
            if score == 0 then
                for _, en in ipairs(exactNames) do
                    if lower == "cur" .. en or lower == "current" .. en then
                        score = 90
                        break
                    end
                end
            ...
            if score == 0 then
                for _, en in ipairs(exactNames) do
                    if lower == en .. "value" or lower == "value" .. en then
                        score = 80
                        break
                    end
                end
...
local function getPlayerChiStam(player)
    ...
    if not chi then
        ...
        if pg then
            chi = searchStatInAttributes(pg, chiNames, exclude)
                or searchStatInRoot(pg, chiNames, exclude)
        end
    end
    if not stam then
        local pg = player:FindFirstChild("PlayerGui")
        if pg then
            stam = searchStatInAttributes(pg, stamNames, exclude)
                or searchStatInRoot(pg, stamNames, exclude)
        end
    end
...
RunService.Heartbeat:Connect(function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        ...
        if silentAim.MaxDistance and silentAim.MaxDistance > 0 and enemyRoot then
            if (enemyRoot.Position - originPos).Magnitude > silentAim.MaxDistance then
                continue
            end
        end
...
local function getTarget()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if settings.maxDistance and settings.maxDistance > 0 and enemyRoot then
            if (enemyRoot.Position - originPos).Magnitude > settings.maxDistance then
                continue
            end
        end
...
local function forceReconnect()
    Fluent:Notify({
        ...
        Content = "Принудительное переподключение...",
        Duration = 3
    })
    ...
    task.spawn(function()
        for i = 1, 3 do
            ...
            if ok then 
                print("[FORCE] Попытка " .. i .. " успешна")
                return 
            end
            print("[FORCE] Попытка " .. i .. " не удалась")
            task.wait(1)
        end
        
        print("[FORCE] Fallback — обычный Teleport")
        pcall(function()
...
local Window = Fluent:CreateWindow({
    Title = "Universal Shindo v8.5",
    SubTitle = "360° • MaxDistance • CHI/STAM • Force Reconnect",
    TabWidth = 160,
...
Tabs.Aimbot:AddToggle("Mode360", {Title = "360° Mode", Description = "Игнорирует FOV и экран", Default = false}):OnChanged(function(v)
    settings.mode360 = v
...
Tabs.Silent:AddToggle("SilentMode360", {Title = "360° Mode", Description = "Игнорирует FOV и экран", Default = false}):OnChanged(function(v)
    silentAim.Mode360 = v
...
Tabs.ESP:AddButton({
    ...
    Description = "Покажет ВСЕ chi/chakra/stam в консоли (F9)",
    Callback = function()
        debugScanPlayer()
...
Tabs.Server:AddButton({
    Title = "🔄 Server Hop",
    Description = "Переход на другой сервер",
...
Tabs.Server:AddButton({
    Title = "⚡ Force Reconnect",
    Description = "Принудительное переподключение (3 попытки + fallback)",
...
print("====================================")
print("✅ Universal Shindo v8.5 загружен!")
print("🎯 Aimbot: MaxDist от персонажа, 360° без onScreen")
print("🎭 Silent Aim: MaxDist, 360° без onScreen")
print("🔍 CHI/STAM: Attributes + PlayerGui (без goto)")
print("🌐 Server: +Force Reconnect")
